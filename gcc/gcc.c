		 ...
		 struct cl_decoded_option *decoded_options)
{
  ...
  */
  ...
  /* If there is any pathname component in an explicit
     -dumpbase, dumpdir is never used.  */
  if (dumpdir && dumpbase && lbasename (dumpbase) != dumpbase)
    {
      free (dumpdir);
      dumpdir = NULL;
      explicit_dumpdir = issue_dumpdir = false;
    }
  ...
  /* If dumpbase was not brought into dumpdir but we're linking, bring
     output_file into dumpdir unless it was explicitly specified.  */
  ...
  /* Now, if we're compiling, then outbase (%B) is derived from
     dumpbase, or from the output name.  */
  else if (have_c)
    {
      if (dumpbase)
	{
	  /* We do not want to drop dirname here; dumpbase with
	     dirnames override dumpdir entirely, and thus we release
	     dumpdir once we find that out.  */
	  if (dumpbase_ext)
	...
	switch (c = *p++)
	  ...
	  case 'B':
	    if (!this_is_output_file && dumpdir_length)
	      obstack_grow (&obstack, dumpdir, dumpdir_length);
	    if (this_is_output_file || !outbase)
	      obstack_grow (&obstack, input_basename,
